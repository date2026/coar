...
/**
 * exec ecpipefg tasks in parallel
 */
void ECWorker::execECPipeFGTasksParallel(AGCommand* agCmd) {
    const std::string filename = agCmd->getFilename();
    int taskNum = agCmd->getEcTaskNum();
    LOG_INFO("execECPipeFGTasksParallel start, filename: %s, taskNum: %d", filename.c_str(), taskNum);
    ...
    for (int i = 0; i < taskNum; i++) {
        ...
        task->parseFG(taskStr);
        LOG_INFO("ECWorker receive task, type: %d, nodeId: %d, srcNodeId: %d, dstNodeId: %d, objId: %d, tmpObjId: %d, objIds: %s, coefs: %s, leftBound: %d, rightBound: %d", 
                ECTaskType2int(task->_type), task->_nodeId, task->_srcNodeId, task->_dstNodeId, task->_objId, task->_tmpObjId, 
                vec2String(task->_objIds).c_str(), vec2String(task->_coefs).c_str(), task->_leftBound, task->_rightBound);
        ...
        switch (tasks[i]->_type) {
            case ECTaskType::SEND:
                execThds[i] = std::thread([=, &svr, &timeMap](){
                    auto ret = execSendECPipeFGTaskParallel(filename, tasks[i], objBuffer, svr);
                    timeMap.setTime(i, ret);
                });
                break;
            case ECTaskType::RECEIVE:
                execThds[i] = std::thread([=, &timeMap](){
                    auto ret = execReceiveECPipeFGTaskParallel(filename, tasks[i], objBuffer);
                    timeMap.setTime(i, ret);
                });
                receiveTaskId++;
                break;
            ...
            case ECTaskType::ENCODE_PARTIAL:
                execThds[i] = std::thread([=, &timeMap](){
                    auto ret = execEncodeECPipeFGTaskParallel(filename, tasks[i], objBuffer);
                    timeMap.setTime(i, ret);
                });
                break;
            case ECTaskType::PERSIST:
                execThds[i] = std::thread([=, &timeMap](){
                    auto ret = execPersistECPipeFGTaskParallel(filename, tasks[i], objBuffer);
                    timeMap.setTime(i, ret);
                });
                break;
            case ECTaskType::FETCH:
                execThds[i] = std::thread([=, &timeMap](){
                    auto ret = execFetchECPipeFGTaskParallel(filename, tasks[i], objBuffer);
                    timeMap.setTime(i, ret);
                });
                break;
            default:
 ...
 */
std::pair<timeval, timeval> ECWorker::execSendECPipeFGTaskParallel(const std::string& filename, const ECTask* task, 
                                                                BlockingQueueParallelBuffer* objBuffer, httplib::Server& svr) {
    ...
    const int leftBound = task->_leftBound;
    const int rightBound = task->_rightBound;
    const int sliceSizeByte = _conf->_sliceSize * 1024 * 1024;
    const int sliceNum = rightBound - leftBound + 1;
    const int sizeByte = sliceNum * sliceSizeByte;

    struct timeval sendStart, sendEnd;

    LOG_INFO("execSendECPipeTaskParallel start, filename: %s, nodeId: %d, dstNodeId: %d, objId: %d, leftBound: %d, rightBound: %d", 
             filename.c_str(), nodeId, dstNodeId, objId, leftBound, rightBound);
 ...
 */
std::pair<timeval, timeval> ECWorker::execFetchECPipeFGTaskParallel(const std::string& filename, const ECTask* task, BlockingQueueParallelBuffer* objBuffer) {
    const int nodeId = task->_nodeId;
    const int objId = task->_objId;
    ...
    const int sliceSizeByte = _conf->_sliceSize * 1024 * 1024;
    const int sizeByte = sliceNum * sliceSizeByte;
    const int startPos = leftBound * sliceSizeByte;
    struct timeval fetchStart, fetchEnd;

    LOG_INFO("execFetchECTaskParallel start, filename: %s, nodeId: %d, odjId: %d, tmpObjId: %d, leftBound: %d, rightBound: %d", 
        filename.c_str(), nodeId, objId, tmpObjId, leftBound, rightBound);
    ...
    for (int i = 0; i < sliceNum; i++) {
        LOG_INFO("execFetchECTaskParallel, filename: %s, nodeId: %d, odjId: %d, tmpObjId: %d, sliceId: %d", 
            filename.c_str(), nodeId, objId, tmpObjId, i);
    ...
    delete [] buf;
    LOG_INFO("execFetchECTaskParallel done, filename: %s, nodeId: %d, odjId: %d, tmpObjId: %d, leftBound: %d, rightBound: %d", 
             filename.c_str(), nodeId, objId, tmpObjId, leftBound, rightBound);
 ...
 */
std::pair<timeval, timeval> ECWorker::execReceiveECPipeFGTaskParallel(const std::string& filename, const ECTask* task, BlockingQueueParallelBuffer* objBuffer) {
    const int nodeId = task->_nodeId;
    const int dstNodeId = task->_dstNodeId;
    const int srcNodeId = task->_srcNodeId;
    const int objId = task->_objId;
    ...
    LOG_INFO("execReceiveECTaskParallel start, filename: %s, nodeId: %d, srcNodeId: %d, objId: %d, tmpObjId: %d, leftBound: %d, rightBound: %d", 
        filename.c_str(), nodeId, srcNodeId, objId, tmpObjId, leftBound, rightBound);
    ...
    LOG_INFO("execReceiveECTaskParallel done, filename: %s, nodeId: %d, srcNodeId: %d, objId: %d, tmpObjId: %d, leftBound: %d, rightBound: %d, receive time: %f ms", 
             filename.c_str(), nodeId, srcNodeId, objId, tmpObjId, leftBound, rightBound, RedisUtil::duration(receiveStart, receiveEnd));
 ...
 */
std::pair<timeval, timeval> ECWorker::execEncodeECPipeFGTaskParallel(const std::string& filename, const ECTask* task, BlockingQueueParallelBuffer* objBuffer) {
    const int nodeId = task->_nodeId;
 ...
 */
std::pair<timeval, timeval> ECWorker::execPersistECPipeFGTaskParallel(const std::string& filename, const ECTask* task, BlockingQueueParallelBuffer* objBuffer) {
    ...
    LOG_INFO("execPersistECPipeTaskParallel start, filename: %s, nodeId: %d, objId: %d, tmpObjId: %d, leftBound: %d, rightBound: %d", 
        filename.c_str(), nodeId, objId, tmpObjId, leftBound, rightBound);
    ...
    LOG_INFO("execPersistECPipeTaskParallel done, filename: %s, nodeId: %d, objId: %d, tmpObjId: %d, leftBound: %d, rightBound: %d, persist time: %f ms", 
            filename.c_str(), nodeId, objId, tmpObjId, leftBound,rightBound, RedisUtil::duration(persistStart, persistEnd));
