...
int main() {
    int fileSizeByte = 1020 * 1024 * 1024;
    const std::string filePath = "/home/openec/lmq_openec/build/input_1020MB_random";
    ...
    memcpy(selectMatrix + 1 * k, matrix + 1 * k, k * sizeof(int));
    memcpy(selectMatrix + 2 * k, matrix + 2 * k, k * sizeof(int));
    memcpy(selectMatrix + 3 * k, matrix + 3 * k, k * sizeof(int));
    memcpy(selectMatrix + 4 * k, matrix + 4 * k, k * sizeof(int));
    memcpy(selectMatrix + 5 * k, matrix + 7 * k, k * sizeof(int));
    ...
    char* data_ptrs_4_decode[] = {data_ptrs[0], data_ptrs[1], data_ptrs[2], data_ptrs[3], data_ptrs[4], coding_ptrs[0]};
    ...
    timeval decode_start, decode_end;
    ...
    for (int i = 0; i < objSizeByte; i++) {
        if (coding_ptrs_4_decode[0][i] != data_ptrs[5][i]) {
