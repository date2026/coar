...
def GenerateECDAGFG(all_node_ids, obj_ids, row_ids, nd, src_node_ids, repair_ratios, help_ratios, matrix, n, k, output):
    ...
    repair_slices = [5, 6, 2, 2, 1, 0, 0, 0] 
    help_slices = [5, 6, 13, 10, 5, 3, 3, 3]   
...
def generate_repair_allocation(n, k, repair, help):
    repair = [5, 6, 2, 2, 1, 0, 0, 0]
    help = [5, 6, 13, 10, 5, 3, 3, 3]
    ...
    while True:
        ...
        print(possible_helpers)
        assert(len(possible_helpers) >= k)
...
#         # select target node
#         for i in range(n):
#             if remaining_repair[i] > 0 and remaining_repair[i] < min_repair:

#                 min_repair = remaining_repair[i]
#                 target_node = i
        
#         # no unmatched repair node
#         if target_node == -1:
#             break
...
#         r = min(selected_helpers[-1][1], remaining_repair[target_node])
#         logging.info(f"taraget_node: {target_node}, help: {selected_helpers}")
#         for j, _ in selected_helpers:
#             distribution[target_node][j] += r
#             assert(remaining_help[j] >= r)
#             remaining_help[j] -= r
#         remaining_repair[target_node] -= r   

#         ec_plan.append((target_node, selected_helpers, r))
...
if __name__ == "__main__":
    n = 8
    k = 3
    repair = [2, 2, 2, 2, 2, 2, 2, 2]
    help = [6, 6, 6, 6, 6, 6, 6, 6]
    print(generate_repair_allocation(n, k, repair, help))
